                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <div class="card">
                            <div class="card-header border-bottom" style="text-align: right;">
                                <button class="btn btn-sm btn-danger" onclick="tambaho()"><i class="ti ti-plus"></i> Tambah Artikel</button>
                            </div>
                            <div class="card-datatable table-responsive">
                                <table id="table-artikel" class="table">
            ...
            <div class="modal-content p-3 p-md-5">
                <div class="modal-body">
                    ...
                    <div class="text-center mb-4">
                        <h3 class="mb-2">Tambah Artikel</h3>
                    </div>
                    <form id="form-data" class="row g-3">
                        ...
                        <div class="col-12 col-md-12">
                            <label class="form-label" for="posting_oleh">Posting Oleh</label>
                            <input type="text" id="posting_oleh" name="posting_oleh" class="form-control" />
                        </div>
                        <div class="col-12 col-md-12">
        ...
        <div class="modal-dialog modal-fullscreen">
            <div class="modal-content p-3 p-md-5">
                <div class="modal-body">
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    <div class="text-center mb-4">
                        <h3 class="mb-2">Edit Artikel</h3>
                    </div>
                    <form id="form-data-edit" class="row g-3">
                        <div class="col-12 col-md-12">
                            <label class="form-label" for="judul_e">Judul Artikel</label>
                            <input type="text" id="judul_e" name="judul_e" class="form-control" />
                            <input type="hidden" id="id_e" name="id_e" class="form-control" />
                        </div>
                        <div class="col-12 col-md-12">
                            <label class="form-label" for="posting_oleh_e">Posting Oleh</label>
                            <input type="text" id="posting_oleh_e" name="posting_oleh_e" class="form-control" />
                        </div>
                        <div class="col-12 col-md-12">
                            <label class="form-label" for="deskripsi_e">Isi Artikel</label>
                            <input type="hidden" name="deskripsi_e" id="deskripsi_e">
                            <div id="editor-2" style="min-height: 160px;">
                            </div>
                        </div>
                ...
                <div class="modal-body">
                    ...
                    <form id="form-data-cover" class="row g-3">
                        <div class="col-12 col-md-12">
                            ...
                            <input type="hidden" id="id_artikel" name="id_artikel" class="form-control" />
                        </div>
...
<script>
    ...
    $(function() {
        $("#table-artikel").DataTable({
            ...
            'ajax': {
                'dataType': 'json',
                'url': '<?= base_url() ?>dashboard/ajax_table_artikel',
                'type': 'post'
            },
            'columns': [{
                    "target": [<?= $target ?>],
                    ...
                    "render": function(data) {
                        return `<img src="<?= base_url('assets/artikel/') ?>` + data.cover + `" alt="artikel" class="img-fluid" style="max-width: 80px;margin-bottom: 10px;"><br>
                                    <button type="button" class="btn btn-sm btn-success waves-effect waves-light" onclick="ubah_cover(` + data.id + `)">Ubah</button>`
                    }
                }, {
                    "target": [<?= $target ?>],
                    "className": 'text-center py-1',
                    "data": "data.posting_oleh",
                }, {
                    "target": [<?= $target ?>],
                    "className": 'text-center py-1',
                ...
                },
                {
                    ...
                    "render": function(data) {
                        return `<div class="d-flex align-items-center">
                                    <a href="javascript:;" class="text-body" onclick="edito('` + data.id + `','` + data.judul + `','` + data.posting_oleh + `','` + data.uraian + `')"><i class="ti ti-edit ti-sm me-2"></i></a>
    ...
    function reload_table() {
        $('#table-artikel').DataTable().ajax.reload(null, false);
    }

    $("#form-data").submit(function(e) {
        e.preventDefault()

        if ($('#judul').val() == '' || $('#posting_oleh').val() == '' || $('#deskripsi').val() == '') {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
        ...
        form_data.append('posting_oleh', $("#posting_oleh").val());
        form_data.append('uraian', $("#deskripsi").val());
        ...
        var url_ajax = '<?= base_url() ?>dashboard/insert_data_artikel'
    ...
    function edito(id, judul, posting_oleh, deskripsi) {
        $('#editbuku').modal('show')

        $('#id_e').val(id)
        $('#judul_e').val(judul)
        $('#posting_oleh_e').val(posting_oleh)
    ...
    $("#form-data-edit").submit(function(e) {
        ...
        if ($('#judul_e').val() == '' || $('#posting_oleh_e').val() == '' || $('#deskripsi_e').val() == '') {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Tidak boleh ada kolom kosong!',
        ...
        form_data.append('judul', $("#judul_e").val());
        form_data.append('posting_oleh', $("#posting_oleh_e").val());
        form_data.append('uraian', $("#deskripsi_e").val());
    ...
    $("#form-data-cover").submit(function(e) {
        ...
        var form_data = new FormData();
        form_data.append('table', 'tbl_artikel');
        form_data.append('id', $("#id_artikel").val());
        ...
        var url_ajax = '<?= base_url() ?>dashboard/ubah_cover_artikel'
